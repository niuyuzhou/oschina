    ...
    if (commentCount > 0) 
    {
        if (row < commentCount) 
        {
            ...
            if (m) {
                ...
                CGFloat fixedWidth = cell.txt_Content.frame.size.width;
                CGSize newSize = [cell.txt_Content sizeThatFits:CGSizeMake(fixedWidth, MAXFLOAT)];
                CGRect newFrame = cell.txt_Content.frame;
                newFrame.size = CGSizeMake(fmaxf(newSize.width, fixedWidth), newSize.height);
                cell.txt_Content.frame = newFrame;
...
- (void)dealloc
{
    [self.tableComments setDelegate:nil];
}
@end
