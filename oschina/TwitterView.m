...
-(void) clickPubTweet:(id)sender
{
    ...
    pubTweet.projectId=self.projectId;
    pubTweet.isSoftWare=YES;
...
-(void) viewWillAppear:(BOOL)animated
{
    ...
    if (self.isSoftware) {
        ...
        self.parentViewController.navigationItem.title =  @"评论列表";
        //将右边按钮设定为 发表评论
        ...
        [btnPubComment setAction:@selector(clickPubTweet:)];
    }
...
- (void)reload:(BOOL)noRefresh
{
    ...
    NSString *url;
    if (self.isSoftware) {
        url= [NSString stringWithFormat:@"%@?project=%d",api_software_tweet_list, self.projectId];
    }
    else{
        url = [NSString stringWithFormat:@"%@?uid=%d&pageIndex=%d&pageSize=%d",api_tweet_list, self._uid,pageIndex, 20];
    }
...
- (void)dealloc
{
    [self.tableTweets setDelegate:nil];
}
