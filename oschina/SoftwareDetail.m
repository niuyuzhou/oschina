...
- (void)viewDidLoad
{
    ...
    if ([Config Instance].isNetworkRunning) {
        ...
        [[AFOSCClient sharedClient] getPath:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
            ...
            self.s = [Tool readStrSoftwareDetail:operation.responseString];
            if (self.s == nil) {
                [Tool ToastNotification:@"加载失败" andView:self.view andLoading:NO andIsBottom:NO];
                return;
            }
            [self loadData:self.s];
...
{
    self.parentViewController.navigationItem.title = @"软件详情";
    if (self.s) {
        [self refreshFavorite:self.s];
    }
...
- (void)loadData:(Software *)s
{
    ...
    //通知去修改软件评论数
    Notification_CommentCount *notification = [[Notification_CommentCount alloc] initWithParameters:self andCommentCount:s.tweetCount projectId:s._id];
    [[NSNotificationCenter defaultCenter] postNotificationName:Notification_DetailCommentCount object:notification];
    // 主要用于微博分享
    [Config Instance].shareObject = [[ShareObject alloc] initWithParameters:s.title andUrl:[NSString stringWithFormat:@"http://m.oschina.net/p/%@",self.softwareName] andContent:s.body];
