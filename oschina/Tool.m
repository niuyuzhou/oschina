...
+ (void)pushNewsDetail:(News *)news andNavController:(UINavigationController *)navController andIsNextPage:(BOOL)isNextPage
{
    switch (news.newsType) {
        ...
        case 1:
        {
            UITabBarController *softTab = [[UITabBarController alloc] init];
            softTab.title = @"软件详情";
            ...
            ShareView *shareView = [[ShareView alloc] init];
            shareView.tabBarItem.image = [UIImage imageNamed:@"share"];
            softTab.viewControllers = [NSArray arrayWithObjects:software,twitterView,shareView, nil];
            softTab.hidesBottomBarWhenPushed = YES;
            [navController pushViewController:softTab animated:YES];
        }
            break;
            //问答
        case 2:
...
+ (void)pushPostDetail:(Post *)post andNavController:(UINavigationController *)navController
{
    ...
    reportView.tabBarItem.image = [UIImage imageNamed:@"report"];
    
    postTab.viewControllers = [NSArray arrayWithObjects:singlePost,commentsView,shareView, reportView, nil];
